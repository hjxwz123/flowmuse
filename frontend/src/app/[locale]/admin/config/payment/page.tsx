...
const inputCls = 'w-full px-4 py-2 border border-stone-300 dark:border-stone-700 rounded-lg focus:ring-2 focus:ring-aurora-purple/30 focus:border-aurora-purple bg-stone-50 dark:bg-stone-900 text-stone-900 dark:text-stone-100 text-sm font-mono transition-colors'
const labelCls = 'block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1.5'
    ...
    >
      <form onSubmit={handleSubmit}>
        <Card className="space-y-6 border border-stone-200 dark:border-stone-800 !bg-stone-50 dark:!bg-stone-900 p-6 !shadow-sm">
          {/* 启用开关 */}
          <div className="flex items-center justify-between p-4 bg-stone-50 dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-800">
            <div>
              <p className="font-semibold text-stone-800 dark:text-stone-200">启用微信支付</p>
              <p className="text-xs text-stone-500 dark:text-stone-400 mt-0.5">关闭后用户无法使用微信扫码购买套餐</p>
            </div>
            <button
              type="button"
            ...
            >
              <span className={`inline-block h-5 w-5 transform rounded-full bg-stone-50 dark:bg-stone-900 shadow transition-transform ${form.enabled ? 'translate-x-8' : 'translate-x-1'}`} />
            </button>
          </div>

          {/* 商户信息 */}
          <div>
            <h2 className="text-base font-semibold text-stone-900 dark:text-stone-100 mb-4 flex items-center gap-2">
              <span className="w-1 h-5 bg-aurora-purple rounded-full inline-block" />
              商户基本信息
            </h2>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <label className={labelCls}>AppID <span className="text-stone-400 dark:text-stone-500 font-normal">(公众号/小程序)</span></label>
          ...
          <div>
            ...
            <div>
              ...
              <p className="text-xs text-stone-500 dark:text-stone-400 mt-1">必须是 HTTPS 地址，微信支付服务器将向此地址发送支付结果通知</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div>
              <label className={labelCls}>商户 API 证书私钥 <span className="text-stone-400 dark:text-stone-500 font-normal">(apiclient_key.pem 文件内容)</span></label>
              <textarea
