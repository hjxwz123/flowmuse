...
| 数据 | MySQL 8、Redis 7 |
| 存储 | 本地存储、腾讯云 COS |
| 媒体处理 | FFmpeg / FFprobe，用于自动项目分镜合并、转场渲染和视频时长探测 |
...
后端 Docker 镜像已内置 `ffmpeg` 和 `ffprobe`。自动项目的分镜合并、视频转场渲染、静音导出和视频时长探测会用到它们；使用 Docker Compose 部署时不需要在宿主机额外安装 FFmpeg。
...
本地源码启动后端时需要系统里可执行 `ffmpeg` 和 `ffprobe`，否则自动项目分镜合并和转场导出会失败。Docker 后端镜像已经内置这两个命令，本地开发环境需要自行安装：
